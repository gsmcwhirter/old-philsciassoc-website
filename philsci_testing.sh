...
git pull
perl -pi -w -e 's!^basepath=~/public_html/generator!basepath=\$HOME/domains/philsci.org/web/public!g' remote_deploy.sh
perl -pi -w -e 's!^targetpath=\$basepath/target!targetpath=\$basepath/staging!g' remote_deploy.sh
perl -pi -w -e 's!^sourcepath=~/sites/PhilSci!sourcepath=\$HOME/dev/PhilSci!g' remote_deploy.sh
./remote_deploy.sh
